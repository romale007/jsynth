...
const whiteNotesFrequencies = [C4.freq, D4.freq, E4.freq, F4.freq, G4.freq, A4.freq, B4.freq,
...
function activePress() {
  ...
  if (whiteNotesKeys.includes(event.code) || blackNotesKeys.includes(event.code)) {
    const pressedElement = document.querySelector(`#box .whitenotes[data-key ='${event.code}'], .blacknotes[data-key ='${event.code}']`);

    const dataset = pressedElement.dataset;
    if (!dataset["pressed"]) {
...
function removeActivePress() {
  if (whiteNotesKeys.includes(event.code) || blackNotesKeys.includes(event.code)) {
    const releasedElement = document.querySelector(`#box .whitenotes[data-key ='${event.code}'], .blacknotes[data-key ='${event.code}']`);

    const dataset = releasedElement.dataset;
    if (dataset["pressed"]) {
